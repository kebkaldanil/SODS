...
async function processRequest<T, P>(o: RequestObject, path: string, sect: RouteBranch<T, P>): Promise<P> {
  try {
    if (sect) {
...
export function createProcessor(routeObj: RouteBranch): RequestListener {
...
export async function file2response(
  o: RequestObject,
  path: string,
  ranges?: string | FileRange[],
  type?: string,
...
export class crudBuilder<T = unknown> {
  private usedMethods: Set<string>;
  private processors: NodeJS.Dict<{ cb: processFunction<T>, priority: number }[]>;
  constructor() {
    this.usedMethods = new Set(["OPTIONS"]);
    this.processors = {
      OPTIONS: [
        {
          cb: o => void o.response.setHeader("Allow", Array.from(this.usedMethods).join(", ")),
          priority: Infinity,
        },
      ],
    };
  }
  useMethod(method: string, cb: processFunction<T>, priority = 50) {
    method = method.toUpperCase();
    if (!this.usedMethods.has(method)) {
      this.usedMethods.add(method);
      this.processors[method] = [];
    }
    this.processors[method].push({ cb, priority });
    return this;
  }
  ...
  post(cb: (path: string, body: string, o: RequestObject) => T, priority = 50) {
  ...
  build(): RouteBranch<T> {
    const res: RouteBranch<T> = {
      [current]: <RouteBranchWithoutResultProcessor<T>>mapArrayToObject(Array.from(this.usedMethods), {
        processor: method => {
