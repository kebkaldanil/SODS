...
import { KeyOfAny, ReadOnlyDict } from "./types";
...
export function dataCopy<T extends object>(obj: T): T {
  const res = {} as T;
  for (const nm in obj) {
    const v = obj[nm];
    switch (typeof v) {
...
export function computeOnce<R, T>(obj: R, key: PropertyKey, compute: () => T): R {
